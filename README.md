...
PotPlayer 自带截图主要用于观看时留图，缺少面向后期工作流的 HDR 色彩解释、16-bit RGB 输出和明确导入标识，素材进入调色或合成软件后容易出现 Gamma、色域或亮度判断错误。FrameClip 直接读取媒体源，不经过桌面捕获和显示器色调映射，依据源色彩特征生成 16-bit RGB PNG/TIFF 参照帧，并可在保留 HDR 原图的同时输出 Rec.709 SDR 副本。围绕同一类素材采集需求，它还提供入点、出点标记，带声音的原码片段与 ProRes/DNxHR 精确片段导出，以及按作品自动归档。
